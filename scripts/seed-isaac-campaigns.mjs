/**
 * Seed Isaac life campaigns (champion id 2).
 * Usage: node --env-file=.env.local scripts/seed-isaac-campaigns.mjs
 */
...
const championId = Number(process.env.ISAAC_CHAMPION_ID || 2);
...
function gymSteps(blockLabel, blockDetail) {
  return [
    ...
    { surface: blockLabel, detail: blockDetail, planned_minutes: 40 },
    {
...
async function insertSteps(missionId, steps) {
...
async function createFullCampaign(def) {
  ...
  for (let ci = 0; ci < def.chapters.length; ci++) {
    ...
    const { data: chapter, error: chErr } = await supabase
      ...
      .insert({
        ...
        objective: ch.objective || def.result,
        order_index: ci,
      })
      .select("id")
      .single();
    ...
    for (let mi = 0; mi < ch.missions.length; mi++) {
      ...
      const { data: mission, error: mErr } = await supabase
        ...
        .insert({
          ...
          title: m.title,
          status: m.status || (ci === 0 ? "available" : "locked"),
          why: m.why || null,
          weekdays: m.weekdays || [],
        ...
        .single();
      if (mErr) throw mErr;
      await insertSteps(mission.id, m.steps);
    }
  }
  return campaign.id;
}

function gymWeek(week) {
  return [
    {
...
const defs = [
  ...
  {
    title: "Leitura — Diário de um mago",
    result:
  ...
  {
    title: "Programação — agência + Dofus",
    result:
  ...
  {
    title: "Finanças — dívidas e disciplina",
...
for (const def of defs) {
  ...
  const campaignId = await createFullCampaign(def);
...
console.log(JSON.stringify({ championId, created, skipped }, null, 2));
