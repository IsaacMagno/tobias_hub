...
import { useRequireChampion } from "../../hooks/useRequireChampion";
...
const page = () => {
  ...
  const { champion, isLoading: isChampionLoading } = useRequireChampion();
          ...
          <span className="flex">
            <ul className="min-w-full">
              {championsProgressList &&
                championsProgressList
                  .filter((champ) => !champ.monthlyChallenge?.completed)
                  .map((champ, index) => (
                    <li className="font-light text-xs" key={index}>
                      <span
