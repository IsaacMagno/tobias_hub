...
const UserStats = ({
  ...
  const statistics = propStatistics ?? champion?.statistics;
  const activities = propActivities ?? champion?.activities;
  const championId = champion?.id;

  const championStatsDetails = async () => {
    if (!championId) return;
    const { statsDetails } = await getStatsDetailsById(championId);
    setStatsDetails(statsDetails);
  };

  useEffect(() => {
    if (championId) championStatsDetails();
  }, [championId]);
